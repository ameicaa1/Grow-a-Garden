...
function functions:Dropdown(text, buttons, optional_height, callback)
    ...
    Dropdown.MouseEnter:Connect(function()
        TweenService:Create(Dropdown, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(55, 55, 55) }):Play()
    end)

    Dropdown.MouseLeave:Connect(function()
        TweenService:Create(Dropdown, TweenInfo.new(0.1), { BackgroundColor3 = Color3.fromRGB(45, 45, 45) }):Play()
    end)

    local DropdownFrame = Instance.new("ScrollingFrame")
    DropdownFrame.Name = "DropdownFrame"
    DropdownFrame.Parent = UiWindow:FindFirstChild("ScreenGui") or game:GetService("CoreGui")
    DropdownFrame.Active = true
    DropdownFrame.BackgroundColor3 = Color3.fromRGB(45, 45, 45)
    DropdownFrame.BorderSizePixel = 0
    DropdownFrame.Size = UDim2.new(0, DropdownContainer.AbsoluteSize.X, 0, 0)
